...
void NilfgaardSoldier::SetStats(const char* json)
{
	...
	if (stat_weapon)
	{
		...
		stats.max_agility = stats.current_agility = stat_weapon->GetNumber("Agility");
...
{
	character_ctrl->SetWalkDirection(direction * stats.current_agility);
	animator->SetFloat("speed", stats.current_agility);
