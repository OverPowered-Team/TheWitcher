...
{
	GameManager::GetInstance()->player_manager->OnPlayerDead(player_dead);
}

void EventManager::OnPlayerRevive(PlayerController* player_revived)
{
	GameManager::GetInstance()->player_manager->OnPlayerRevive(player_revived);
}
...
void EventManager::ReceiveDialogueEvent(Dialogue& dialogue, float delay) const
{
	...
	GameManager::GetInstance()->dialogue_manager->InputNewDialogue(dialogue);
...
void EventManager::ReceiveDialogueEvent(int index, float volume) const
{
	...
	GameManager::GetInstance()->dialogue_manager->InputNewDialogue(index, volume);
}
